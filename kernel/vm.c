...
static void vmPrintPTE(int index, pte_t pte, int level) {
  printf("..");
...
static void vmPrintTable(pagetable_t pgtbl, int level) {
  if (level < 0) {
    return;
  }
  ...
  for (int i = 0; i < 512; i++) {
    ...
    if (pte & PTE_V) {
      uint64 child = PTE2PA(pte);
      vmPrintPTE(i, pte, level);
      vmPrintTable((pagetable_t)child, level - 1);
    }
  }
}
